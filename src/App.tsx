      ...
      <svg width="0" height="0" style={{ position: 'absolute' }}>
        <defs>
          <linearGradient id="vivid-icon-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <stop offset="100%" stopColor="#7d4673" />
          </linearGradient>
        </defs>
      </svg>
        ...
        <div className="flex flex-wrap lg:flex-nowrap items-center justify-between gap-4 w-full relative z-10">
          <div className="flex items-center gap-3 shrink-0">
            <motion.div 
              ...
              className="w-10 h-10 bg-[#2563eb] rounded-lg flex items-center justify-center relative overflow-hidden shadow-sm"
            >
              <Fuel className="w-6 h-6 text-white relative z-10" />
            </motion.div>
            <div>
              <h1 className="text-xl font-extrabold text-[#2563eb] tracking-tight leading-none">MODULA</h1>
              <div className="flex items-center gap-1.5 mt-1">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_5px_#10b981]" />
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Sistema Online</span>
              </div>
            </div>
          </div>
            ...
            <>
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input 
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-md bg-white border border-gray-300 focus:border-[#2563eb] focus:ring-1 focus:ring-[#2563eb] outline-none transition-all font-medium text-sm text-slate-700 h-[42px] shadow-sm"
                />
              </div>
              ...
              <button 
                ...
                className={cn(
                  "px-4 py-2.5 rounded-md border transition-all font-semibold text-xs uppercase tracking-wider flex items-center justify-center gap-2 h-[42px]",
                  showAdvancedFilters ? "bg-[#2563eb] text-white border-[#2563eb] shadow-sm" : "bg-white text-slate-600 border-gray-300 hover:bg-gray-50"
                )}
              >
              ...
              <button 
                ...
                className="px-4 py-2.5 rounded-md bg-white border border-gray-300 text-slate-400 hover:text-[#2563eb] hover:bg-gray-50 transition-all font-medium text-sm flex items-center justify-center gap-2 h-[42px] shadow-sm"
                title="Reset filtri"
              >
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              disabled={loading}
              className={cn(
                "px-3 py-2 rounded-md transition-all flex items-center gap-2 font-bold text-[10px] tracking-widest uppercase border shadow-sm",
                view === 'converter' ? "bg-blue-50 text-blue-600 border-blue-200" : "bg-white text-slate-500 hover:bg-gray-50 border-gray-300",
                loading && "opacity-50 cursor-not-allowed"
              )}
            >
              <FileSpreadsheet className="w-4 h-4" />
              <span className="hidden sm:inline">Importazione</span>
            </button>

            <div className="flex bg-gray-100 p-1 rounded-md border border-gray-200 shadow-inner hidden sm:flex">
              <button
                onClick={() => setView('map')}
                disabled={loading}
                className={cn(
                  "px-3 py-1.5 rounded-md text-[10px] tracking-widest uppercase font-bold transition-all flex items-center gap-1.5",
                  view === 'map' ? "bg-white text-[#2563eb] shadow-sm border border-gray-200" : "text-slate-500 hover:text-slate-700 transparent border border-transparent",
                  loading && "opacity-50 cursor-not-allowed"
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "px-3 py-1.5 rounded-md text-[10px] tracking-widest uppercase font-bold transition-all flex items-center gap-1.5",
                  view === 'list' ? "bg-white text-[#2563eb] shadow-sm border border-gray-200" : "text-slate-500 hover:text-slate-700 transparent border border-transparent",
                  loading && "opacity-50 cursor-not-allowed"
                )}
              >
            ...
            <button 
              ...
              className={cn(
                "px-3 py-2 rounded-md transition-all flex items-center gap-2 font-bold text-[10px] tracking-widest uppercase border shadow-sm",
                showDashboard ? "bg-[#2563eb]/10 text-[#2563eb] border-[#2563eb]/30" : "bg-white text-slate-500 hover:bg-gray-50 border-gray-300",
                loading && "opacity-50 cursor-not-allowed"
              )}
            >
            ...
            <button
              ...
              disabled={loading}
              className={cn(
                "relative p-2 rounded-md transition-all border shadow-sm",
                alertCount > 0 ? "bg-rose-50 text-rose-600 border-rose-200" : "bg-white text-slate-400 border-gray-300 hover:bg-gray-50",
                loading && "opacity-50 cursor-not-allowed"
              )}
            ...
            >
              ...
              {alertCount > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 min-w-[16px] min-h-[16px] bg-rose-500 text-white text-[9px] font-black rounded-full flex items-center justify-center leading-none px-0.5 shadow-sm">
            ...
            <button 
              ...
              className="p-2 bg-white text-slate-400 hover:text-rose-500 hover:bg-gray-50 rounded-md transition-all border border-gray-300 flex items-center justify-center gap-1 shadow-sm"
            >
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {[...Array(10)].map((_, i) => (
                  <div key={`skeleton-${i}`} className="bg-white border border-gray-100 shadow-sm flex flex-col justify-between h-[160px] p-4 relative overflow-hidden rounded-lg animate-pulse">
                    <div className="space-y-3">
                      ...
                      <Skeleton className="w-1/2 h-2 bg-gray-100" />
                    </div>
                    <div className="space-y-2">
                      <div className="w-full h-1.5 bg-gray-50 rounded-full overflow-hidden">
                        <div className="h-full bg-gray-200 w-1/2" />
                      </div>
                      <div className="w-full h-1.5 bg-gray-50 rounded-full overflow-hidden">
            ...
            >
              <Suspense fallback={<div className="p-8 h-full bg-slate-900/20 backdrop-blur-md rounded-2xl border border-white/5 flex items-center justify-center font-mono text-blue-500/50 text-[10px] tracking-[0.3em] uppercase animate-pulse">Caricamento Protocollo...</div>}>
                <ExcelConverter />
              </Suspense>
            </motion.div>
          ) : (
            ...
            >
              ...
              <div className="bg-white p-8 rounded-lg border border-gray-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-8 mt-8">
                <div className="flex flex-wrap justify-center gap-8 md:gap-16">
                  <div className="text-center">
                    <p className="text-3xl md:text-4xl font-sans font-bold text-slate-800 mb-1">{filteredInstallations.length}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Nodi Attivi</p>
                  </div>
                  <div className="text-center">
                    <p className="text-3xl md:text-4xl font-sans font-bold text-[#2563eb] mb-1">
                      {filteredInstallations.reduce((acc, i) => acc + i.rows.filter(r => r["ID Serbatoio"] && r["ID Serbatoio"] !== "-").length, 0)}
                    </p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Serbatoi</p>
                  </div>
                  <div className="text-center">
                    <p className="text-3xl md:text-4xl font-sans font-bold text-emerald-600 mb-1">
                      {filteredInstallations.reduce((acc, i) => acc + i.rows.filter(r => r["ID Erogatore"] && r["ID Erogatore"] !== "-").length, 0)}
                    </p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Erogatori</p>
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                  <motion.button 
                    ...
                    className="flex-1 md:flex-none uppercase tracking-widest text-[10px] bg-white text-[#2563eb] hover:bg-[#2563eb] hover:text-white font-bold px-6 py-3 border border-[#2563eb] rounded-md flex items-center justify-center gap-2 shadow-sm transition-all"
                  >
                    <Download className="w-4 h-4" /> Esporta DOC
                  </motion.button>
                  <motion.button 
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    onClick={handleExportCSV}
                    className="flex-1 md:flex-none uppercase tracking-widest text-[10px] bg-[#2563eb] text-white hover:bg-[#1e40af] font-bold px-6 py-3 border border-[#2563eb] rounded-md flex items-center justify-center gap-2 shadow-sm transition-all"
                  >
