...
export function calculateHealthScore(
  inst: Installation,
  maxSell: number = 200000 // Added default
): HealthResult {
  ...
  else if (inst.ebitda > -50000) {
    ebitdaScore = 10;
    reasons.push("EBITDA in contrazione");
  } else {
    ebitdaScore = 0;
    reasons.push("EBITDA critico (negativo)");
  }
  ...
  if (!hasExpired && !hasExpiringSoon) {
    ...
    inspectionScore = 20;
    reasons.push("Verifica Biennale in scadenza");
  } else {
    inspectionScore = 0;
    reasons.push("Verifica Biennale SCADUTA");
  }
  ...
  if (sellRatio < 0.2) {
    reasons.push("Volumi di vendita sotto la media");
  }
  ...
  return { 
    ...
    borderColor, 
    emoji, 
    details: { ebitdaScore, inspectionScore, sellScore },
    reasons
  };
}
